...
def load_sensors_location_data(apps, schema_editor):
    sensors_location = gpd.read_file('./agrogeoinfoapp/static/data/ferme/spatial/SensorsLocation.geojson')
    sensors_location = sensors_location.to_crs(3857)
    sensors_location['wkt'] = sensors_location.geometry.to_wkt()
    ...
    for sensor in sensors_location.itertuples():
        parcel_code = sensor.Strip
        field_code = sensor.Field
        geotype = sensor.geometry.type[0]  # to convert the Geometry type to match the CV in the ODM
        ...
        parcel_feature = SamplingFeatures(
                ...
                featuregeometry=GEOSGeometry(str(sensor.geometry)),
                featuregeometrywkt=str(sensor.wkt),
        ...
        related_feature = SamplingFeatures.objects.filter(samplingfeaturecode=f"Parcel{parcel_code}_Field{field_code}").first()
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("agrogeoinfoapp", "0006_auto_20250612_1522"),
    ]
